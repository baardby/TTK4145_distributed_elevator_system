		...
		for floor := 0; floor < N_FLOORS; floor++ {
			for btn := ButtonType(0); btn < N_BUTTONS; btn++ {
				...
				if newButtonPress != prevButtonPress[floor][btn] && newButtonPress != false {
					btnQueue.EnqueueButtonEvent(ButtonEvent{floor, ButtonType(btn)})
				}
				prevButtonPress[floor][btn] = newButtonPress
			}
		}
		if !btnQueue.IsEmpty() {
			btnToBeSent := btnQueue.DequeueButtonEvent()
			buttonPressEvent <- btnToBeSent
		}
	}
}
